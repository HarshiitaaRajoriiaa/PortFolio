   ...
   return (
      <Box ref={innerRef} component={'main'} display={'flex'} flexDirection={{xs: 'column', md: 'row'}} alignItems={'center'}
         ...
         <Box className={classNames(Style.avatar, Style.shadowed)} alt={'image of developer'} style={{background: info.gradient}} component={'img'} src={me} width={{xs: '35vh', md: '50vh'}}
              ...
              borderRadius={'20%'} p={'0.75rem'} mb={{xs: '1rem', sm: 0}} mr={{xs: 0, md: '15rem'}} mt={{xs: 0, md: '2rem'}}/>
         <Box  mt={'7rem'}>
            <h1>Hi, I'm <span  className={Style.hand} style={{background: info.gradient, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' , }}>{info.firstName}</span>
            </h1>
            <h2 mt={'7rem'}>I'm {info.position}.</h2>
            <Box component={'ul'} p={'3rem'} fontSize={'5px'}>
